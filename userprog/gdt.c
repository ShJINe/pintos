...
gdt_init (void)
{
  ...
  /* Initialize GDT. */
  gdt[SEL_NULL / sizeof *gdt] = 0;                        // 0/8=0 gdt表第一项为空
  gdt[SEL_KCSEG / sizeof *gdt] = make_code_desc (0);      // 8/8=1
  gdt[SEL_KDSEG / sizeof *gdt] = make_data_desc (0);      //16/8=2
  gdt[SEL_UCSEG / sizeof *gdt] = make_code_desc (3);      //27/8=3
  gdt[SEL_UDSEG / sizeof *gdt] = make_data_desc (3);      //35/8=4
  gdt[SEL_TSS / sizeof *gdt] = make_tss_desc (tss_get ());//40/8=5

  /* Load GDTR, TR.  See [IA32-v3a] 2.4.1 "Global Descriptor
     Table Register (GDTR)", 2.4.4 "Task Register (TR)", and
     6.2.4 "Task Register".  */
  gdtr_operand = make_gdtr_operand (sizeof gdt - 1, gdt); //构建gdt寄存器中的内容格式
  asm volatile ("lgdt %0" : : "m" (gdtr_operand)); //gdt base:limit装入gdtr寄存器
  asm volatile ("ltr %w0" : : "q" (SEL_TSS)); //TSS选择子装入tr寄存器
}

